...
class UnitNameDialog(QDialog):
    ...
    def __init__(self, database, unit):
        ...
        self.ward.setGeometry(QtCore.QRect(660, 203, 221, 20))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.description.setGeometry(QtCore.QRect(60, 660, 411, 111))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(11)
        font.setBold(False)
        ...
        # unit = self.database.get_unit_by_slot(self.slot_number)
        unit = self.database.get_unit_by_name(self.unit)
...
class UnitSlotDialog(QDialog):

    def __init__(self, database, slot_number):
        super().__init__()
        self.database = database
        self.slot_number = slot_number
        # self.unit = unit
...
class EnemyUnitSlotDialog(QDialog):
    ...
    def __init__(self, database, slot_number):
        ...
        unit = self.database.get_dungeon_unit_by_slot(self.slot_number)
        # unit = self.database.get_unit_by_name(self.unit)
        unit_frame = QPixmap(UNIT_FRAME)
        portrait_img = QPixmap(f"{PORTRAITS}{unit.name}.gif")
        self.background.setPixmap(unit_frame)
